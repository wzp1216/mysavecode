...
def empty(a):
    ...
    with open('./tool_values.json', 'w') as json_file:
        json_file.write(json_str)
    json_file.close()
    return h_min, h_max, s_min, s_max, v_min, v_max ,blur, logyex, area


with open('./tool_paraments.json', 'r', encoding='utf-8') as json_file:
...
while True:
    ...
    for istat in stats:
        ...
        i = i + 1
    # 绘制轮廓
    contours,_ = cv2.findContours(canny_binary_black, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)   # 输出所有轮廓cv2.RETR_TREE，
