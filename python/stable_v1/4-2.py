...
class AI_TEST(object):
    ...
    # 备用信息，若未识别到，则执行该数据
    mission = json.loads(
        '{"Grab":{"eggplant_box":1,"eggplant_box1":1,"watermelon_box":2,"watermelon_box1":2},"Grab_area_leftrear":[14.290,7.454],"Grab_area_rightfront":[25.501,17.572]'+
    ...
    def GrabLand(self,x , y , z , yaw = 0 , land_height = 0,target_class = "apple_box"):
        ...
        if drone.state.armed and drone.local_pose.pose.position.z > 0.5:
            ...
            timeout = 0 # 超时计次变量
            tag = 0 # 红外tag发现累计次数变量
            target_true = 10 # 需要抓取的目标物体名称，识别正确数量
            '''
            for i in range(15):
                for i in range(4)
    ...
    def start_mission(self):
        ...
        if "watermelon_box" in self.mission["Grab"]:
            try:print("watermelon_box位置X：" + str(drone.watermelon_box_pose1[0]) + " Y："+ str(drone.watermelon_box_pose1[1]))
        ...
        for i in range(len(self.mission["Grab"])):
            if "eggplant_box" in self.mission["Grab"] and (eggbox < 1) and (drone.eggplant_box_pose != None):
                ...
                self.GrabLand(drone.watermelon_box_pose[0],drone.watermelon_box_pose[1],self.Grab_z,yaw = 0,target_class = "pear_box")
                time.sleep(3)
                io.setServoPulse(0, self.close)
                drone.mqtt.push_pwm("grab")
                ...
                watbox=watbox+1
                print("降落至watermelon box1")
                self.GrabLand(drone.watermelon_box_pose1[0],drone.watermelon_box_pose1[1],self.Grab_z,yaw = 0,target_class = "pear_box")
                time.sleep(3)
                io.setServoPulse(0, self.close)
                drone.mqtt.push_pwm("grab")
                self.launch(self.mission["Grab"]["watermelon_box"])
...
if __name__ == '__main__':
    ...
    io = MultiFunction_IO()
    io.setPWMFreq(50)
    drone = SpotController(target=["eggplant_box","watermelon_box"])
    ai.start_mission()
