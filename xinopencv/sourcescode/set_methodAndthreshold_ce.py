...
# Form implementation generated from reading ui file '单独设置侧面检测方法和阈值对话框1108.ui'
#
...
class Ui_Dialog(object):
    ...
    def yes(self):
        method_ce = self.comboBox_5.currentText()
        yzfg_ce = self.comboBox_4.currentText()
        value_ce = self.spinBox.value()
        c_ce = self.spinBox_2.value()
        block = self.spinBox_3.value()
        min_ce = self.doubleSpinBox.value()
        ...
        data['c_ce'] = str(c_ce)
        data['method_ce'] = str(method_ce)
        data['yzfg_ce'] = str(yzfg_ce)
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label_33.setText(_translate("Dialog", "侧面检测方法："))
        ...
        self.label_34.setText(_translate("Dialog", "侧面阈值分割参数设置："))
        self.label_35.setText(_translate("Dialog", "侧面阈值："))
        self.label_36.setText(_translate("Dialog", "侧面最小缺陷阈值："))
...
class MyMainWindow_dialog1(QMainWindow, Ui_Dialog):
    def __init__(self, parent=None):
        ...
        with open('./paraments/paraments.json', 'r', encoding='utf-8') as json_file:
            data = json.load(json_file)
        if 'value_ce' not in data or 'c_ce' not in data or 'min_ce' not in data or 'method_ce' not in data or 'yzfg_ce' not in data or 'block' not in data:
        ...
        json_file.close()
        print(value_ce)
        self.comboBox_5.setCurrentText(method_ce)
        self.comboBox_4.setCurrentText(yzfg_ce)
        self.spinBox.setValue(int(value_ce))
        self.spinBox_2.setValue(float(c_ce))
        self.spinBox_3.setValue(int(block))
        self.doubleSpinBox.setValue(float(min_ce))
